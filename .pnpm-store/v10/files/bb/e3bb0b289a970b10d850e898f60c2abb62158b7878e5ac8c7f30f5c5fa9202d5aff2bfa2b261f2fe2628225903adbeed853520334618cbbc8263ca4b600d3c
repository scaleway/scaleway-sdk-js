...
function hasUpperBound(range) {
  range = new semver__namespace.Range(range);
...
function upperBound(range) {
  ...
  return range.set.map(function (subset) {
    ...
    return subset.sort(function (a, b) {
      return semver__namespace.compare(a.semver, b.semver);
    })[0];
  }).sort(function (a, b) {
    return semver__namespace.rcompare(a.semver, b.semver);
...
function getBoundsForRange(range) {
  return new semver__namespace.Range(range).set.map(function (comparatorSet) {
...
function compareBounds(boundA, boundB) {
  ...
  var compA = new semver__namespace.Comparator(boundA);
  var compB = new semver__namespace.Comparator(boundB);
  var versionComparison = semver__namespace.compare(compA.semver, compB.semver);
...
function highest(rawRanges) {
  ...
  rangesWithBounds.sort(function (a, b) {
    var compA = new semver__namespace.Comparator(a.lowerBound);
    var compB = new semver__namespace.Comparator(b.lowerBound);
    if (semver__namespace.eq(compA.semver, compB.semver)) {
      if (compA.operator === compB.operator) {
        return 0;
      }
      return /=/.test(compA.operator) ? 1 : -1;
    }
    return semver__namespace.compare(compA.semver, compB.semver);
  });
  ...
  rangesWithHighestLowerBound.sort(function (a, b) {
    ...
    var compA = new semver__namespace.Comparator(a.upperBound);
    var compB = new semver__namespace.Comparator(b.upperBound);
    if (semver__namespace.eq(compA.semver, compB.semver)) {
      if (compA.operator === compB.operator) {
        return 0;
      }
