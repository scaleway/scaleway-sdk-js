...
const makeJSONResponse = (value: JSON | null = SIMPLE_REQ_BODY, status = 200) =>
  makeResponse(value, status, 'application/json')

const makeTextResponse = (value: string | null, status = 200) =>
  makeResponse(value, status, 'plain/text')
...
describe(`responseParser`, () => {
  ...
  it(`returns the response as-if for unknown content type`, async () => {
    const textResponse = makeTextResponse('text-body')
