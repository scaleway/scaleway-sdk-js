...
export interface Offer {
   ...
   *
   * One-of ('offerInfo'): at most one of 'serverInfo', 'serviceLevelInfo',
   * 'rpnInfo', 'sanInfo', 'antidosInfo', 'backupInfo', 'usbStorageInfo',
   * 'storageInfo', 'licenseInfo', 'failoverIpInfo', 'failoverBlockInfo',
   * 'bandwidthInfo' could be set.
   */
...
export interface OS {
  ...
  /**
   * If both requires_valid_hostname & hostname_regex are set, it means that at
   * least one of the criterias must be valid.
   */
...
export interface RpnSan {
  ...
  /**
   * True if the offer supports the RPNv1 implicitly, false if it must to be
   * added to a group to support RPNv1.
   */
...
export type ListFailoverIPsRequest = {
  ...
  search?: string
  /**
   * True: return all failovers IP not attached on server false: return all
   * failovers IP attached on server.
   */
